...
//        }
        //update by pine@20190617
        workerId = workerIdAssigner.assignWorkerId() % bitsAllocator.getMaxWorkerIdMod();
    ...
    public String parseUID(long uid) {
        ...
        //return Long.parseLong(dayInt) * 1000000000000L + ((daySeconds << timestampShift) | (workerId << workerIdShift) | sequence);
        // parse UID
        long dayInt = uid / 10000000000000L;
        long uidInt = uid % 10000000000000L;
        long sequence = (uidInt << (totalBits - sequenceBits)) >>> (totalBits - sequenceBits);
        long workerId = (uidInt << (timestampBits + signBits)) >>> (totalBits - workerIdBits);
        long deltaSeconds = uidInt >>> (workerIdBits + sequenceBits);

//        Date thatTime = new Date(TimeUnit.SECONDS.toMillis(epochSeconds + deltaSeconds));
        Date thatTime = new Date(DateUtils.parseByYearDayIntPattern("20" + String.valueOf(dayInt)).getTime() + TimeUnit.SECONDS.toMillis(deltaSeconds));
